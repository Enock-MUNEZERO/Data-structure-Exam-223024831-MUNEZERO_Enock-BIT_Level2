...
class MinMetric : public GradeMetric {                    // Inherits from the abstract base class GradeMetric
public:
    float compute(const Student* student) override {      // Override base compute method
        if (student->nGrades == 0) return 0.0f;           // Return 0 if no grades available
...
int main() {
    ...
    int totalGrades;                          //Variable to store how many grades will be entered
    cout << "Enter number of grades: ";
    cin >> totalGrades;                    

    // Input each grade
    for (int i = 0; i < totalGrades; ++i) {
        float grade;                                 //Temporary variable to store user input
    ...
    // Create metrics
    GradeMetric* metrics[2];                        //Array of pointers to GradeMetric (interface) for polymorphism
    ...
    for (int i = 0; i < student.nGrades; ++i)                   // Display each grade
        cout << *(student.grades + i) << " ";                   // Output each grade using pointer
    cout << fixed << setprecision(2) << endl;                   // Format: show 2 digits after decimal
